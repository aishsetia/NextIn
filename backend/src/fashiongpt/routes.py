...
async def execute(db_session: DBSession):
    clothing_items = await db_session.exec(select(ClothingItem).where(ClothingItem.status == ClothingItemStatus.PROCESSING).order_by(ClothingItem.created_at.desc()))
...
async def suggest(db_session: DBSession, prompt: str):
    available_items = await db_session.exec(select(ClothingItem).where(ClothingItem.status == ClothingItemStatus.FINISHED))
